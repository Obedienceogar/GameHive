...
async def start_game_function(waitingroom,id):
...
async def elim_start_game_function(waitingroom,id): # this is the start game function for the elimination trivia game
    start_time = time.time()
...
def get_proxy_list():
    try:
        ...
        print(f"Request failed while trying to fetch proxy\nError was: {e}")
    print("The proxy list function is returning NONE")
    return None
...
def return_available_games():
    ...
    available_games_keyboard.row(triviagames)
