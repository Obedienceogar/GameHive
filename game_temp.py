...
class EliminationTriviaGame():
    ...
    async def game(self): # This will keep track of the time and then call the next round method once the timer is up
        while self.players_list and self.return_game_status() == True:
            ...
            if self.timer <= 1:
                self.temp_iterate = copy.deepcopy(self.players_list)
                print(f"This is the value stored in the temporary iteration variable {self.temp_iterate}")
                for i in self.temp_iterate: # basically this block of code is responsible for eliminating players that delay
                    # in answering a question
                    print("iteratedooooooooooooooooo ")
                    if i in self.user_answered: # self.answered is the attribute list that stores all the id of the users that answered in the current round
                        pass
                    else:
                        self.elim_users.append(i)
    ...
    async def analyze_round(self): # This function will be responsible for analyzing each round, determining fi a user should be eliminated
        ...
        for i in self.players_list[:]: # will be accessing the list of all players in the game to determine if the user should be eliminated
            for j in self.opt: # loop through self.options variable per each player that the previous loop temporary variable gets
                self.flag = self.users_choice[j].endswith("🟢")
                if self.flag == True: # if the self.flag variable is equal to True that means that the user answered the question in the current round
                    self.temp_option = self.users_choice[j] # This will store the option selected by the user since j is the temproary variable that contains the option selected by the user
                    if self.temp_option.lower() == self.normal_option_and_answer['correct_answer'].lower():
...
class TriviaGame():
    ...
    async def game(self): # This will keep track of the time and then call the next round method once the timer is up
        while self.players_list:
            ...
            await self.update_message() # this calls the update_message() method to edit the message to reflect changes in time and print the message to the user
            if self.timer <= 0:
                for i in self.players_list[:]: # basically this block of code is responsible for eliminating players that delay
                    # in answering a question
                    if i in self.user_answered: # self.answered is the attribute list that stores all the id of the users that answered in the current round
                        pass
                    else:
